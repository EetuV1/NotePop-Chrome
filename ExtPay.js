                ...
                if (
                    ...
                    const wrapAPIs = (extensionAPIs) => {
                        ...
                        const makeCallback = (promise, metadata) => {
                            return (...callbackArgs) => {
                                if (extensionAPIs.runtime.lastError) {
                                    promise.reject(
                                        extensionAPIs.runtime.lastError
                                    )
                                } else if (
                        ...
                        const onMessageWrappers = new DefaultWeakMap(
                            (listener) => {
                                ...
                                return function onMessage(
                                    ...
                                    let sendResponsePromise = new Promise(
                                        (resolve) => {
                        ...
                        const wrappedSendMessage = (
                            ...
                            return new Promise((resolve, reject) => {
                                const wrappedCb =
                                    wrappedSendMessageCallback.bind(null, {
                                        resolve,
                                        reject,
                                    })
                                args.push(wrappedCb)
    ...
    if (typeof window !== "undefined") {
        window.addEventListener(
            ...
            (event) => {
                ...
                if (
                    event.data === "fetch-user" ||
                    event.data === "trial-start"
                ) {
...
`
        ...
        async function create_key() {
            ...
            if (browserPolyfill.management) {
                ...
                ext_info = await browserPolyfill.runtime.sendMessage(
                    "extpay-extinfo"
                ) // ask background page for ext info
                if (!ext_info) {
        ...
        async function fetch_user() {
            ...
            const resp = await fetch(
                `${EXTENSION_URL}/api/user?api_key=${api_key}`,
                {
                    method: "GET",
                    headers: {
                        Accept: "application/json",
            ...
            parsed_user.installedAt = new Date(
        ...
        async function open_popup(url, width, height) {
            if (browserPolyfill.windows && browserPolyfill.windows.create) {
                const current_window =
                    await browserPolyfill.windows.getCurrent()
        ...
        ${content_script_template}`
            ...
            startBackground: function () {
                browserPolyfill.runtime.onMessage.addListener(function (
                    ...
                    // console.log(
                    //     "service worker got message! Here it is:",
                    //     message
                    // )
...
// Added here by a NotePop creator
